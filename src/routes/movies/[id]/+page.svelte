<script lang="ts">
	...
	import { movieSchema, updateMovie } from '$lib/modules/movies';
	...
	const createUpdateMovieMutation = createMutation({
		mutationKey: ['movies'],
	...
	const form = superForm(defaults(zod(movieSchema)), {
		...
		onUpdate({ form }) {
			if (form.valid) {
				$createUpdateMovieMutation.mutate({ movieID: data.movie.movie.id, data: form.data });
			}
		}
	});
	...
	$formData.title = data.movie.movie.title;
	$formData.year = data.movie.movie.year;
	$formData.runtime = data.movie.movie.runtime;
	$formData.genres = data.movie.movie.genres;
